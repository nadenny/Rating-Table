...
When you change `index.html` or icons, bump the cache version in `sw.js` (e.g. `rating-table-v1` → `v2`) so devices pull the new version instead of serving the old cached one.
