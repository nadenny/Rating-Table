...
1. Make sure the repo contains an empty file named **`.nojekyll`** in the root. This disables GitHub's Jekyll build, which can otherwise skip static files. Create it directly on github.com: **Add file → Create new file**, name it exactly `.nojekyll`, commit. (Dotfiles are easy to miss when dragging a folder into the uploader, so create it by hand.)
2. Confirm `manifest.webmanifest` really is at the root with that exact name (not `manifest.json`, not `manifest.webmanifest.txt`).
3. Wait ~1 minute for Pages to rebuild, then reload `https://<you>.github.io/<repo>/manifest.webmanifest` — it should show JSON.
4. Delete the old home-screen icon and reinstall from the live URL.
